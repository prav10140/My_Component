...
if st.sidebar.button("Solve V=IR"):
    if v_val > 0 and r_val > 0:
        st.sidebar.success(f"I = {v_val / r_val:.2f} A")
    elif i_val > 0 and r_val > 0:
        st.sidebar.success(f"V = {i_val * r_val:.2f} V")
    elif v_val > 0 and i_val > 0:
        st.sidebar.success(f"R = {v_val / i_val:.2f} Ω")
    else:
...
if canvas_result.json_data:
    ...
    if rects and st.button("🔍 Analyze Selection"):
        ...
        # 2. ACCURATE BLACK & WHITE SHARPENING
        # This makes the lines 100% white and background 100% black
        ...
        for i, rect in enumerate(rects):
            ...
            # 4. DRAW RESULTS ON ORIGINAL
            draw.rectangle([l, t, l+w, t+h], outline="red", width=3)
            draw.text((l + 5, t + 5), f"{label}", fill="red")
            
            with cols[i % 4]:
                st.image(crop, caption=f"AI Crop {i+1}", use_column_width=True)
                st.write(f"**{label}** ({conf:.1%})")

        st.divider()
        st.subheader("Annotated Circuit Design")
